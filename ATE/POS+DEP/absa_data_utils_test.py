...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, mode):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        doc=nlp(' '.join(example.text_a))
        list_pos=[]
        list_dep=[]

        for tok in doc:
            list_pos.append(tok.pos_)
            list_dep.append(tok.dep_)

        dep_pos_sent=' '.join(list_dep)+' '+' '.join(list_pos)

        # train_pos.append(' '.join(list_pos))
        example.start=len(tokenizer2.tokenize(dep_pos_sent))+2

        final_encode=tokenizer2.encode_plus(dep_pos_sent,' '.join(example.text_a),max_length=max_seq_length,pad_to_max_length=True)
        input_ids=final_encode['input_ids']
        segment_ids=final_encode['token_type_ids']
        input_mask=final_encode['attention_mask']
        ...
        if mode!="ae":
            ...
            label_id[(len(tokenizer2.tokenize(dep_pos_sent))+2):len(lb)+(len(tokenizer2.tokenize(dep_pos_sent))+2)] = lb
