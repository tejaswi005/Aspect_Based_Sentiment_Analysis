...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, mode):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        example.start=len(tokenizer2.tokenize(' '.join(list_pos)))+2

        final_encode=tokenizer2.encode_plus(' '.join(list_pos),' '.join(example.text_a),max_length=max_seq_length,pad_to_max_length=True)
        ...
        if mode!="ae":
            ...
            label_id[(len(tokenizer2.tokenize(' '.join(list_pos)))+2):len(lb)+len(tokenizer2.tokenize(' '.join(list_pos)))+2] = lb
